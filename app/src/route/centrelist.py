...
import requests
from app.src.model.geo import geocode_centremap  # <- your existing function (dict-based)
...
def centrelist(city=None):
    ...
    """
    # 1) Load centres (optionally filtered by city)
    ...
    # 2) Choose current city label (for header + map centre)
    current_city = city
    if not current_city:
        current_city = centres[0]["city_name"] if centres and centres[0].get("city_name") else "Christchurch"

    # 3) Geocode city for map centre (fallback: Christchurch)
    ...
    # 4) Build centres_for_map with sequential numbers + geocoded coords
    centres_for_map = []
